...
@Component({
  selector: 'app-contract-provision-remain-page',
  templateUrl: './contract-provision-remain-page.component.html',
  styleUrls: ['./contract-provision-remain-page.component.css']
})
export class ContractProvisionRemainPageComponent implements OnInit {
  btnclicked = false;
  type: string;
  OverPercentWidth;
  ...
  FillMainGrid() {
    ...
    if (!isUndefined(this.RegionParams.selectedObject)) {
      this.ProductRequest.GetProductRequestWithRemainProvision(this.RegionParams.selectedObject).subscribe(res => {
        this.rowData = res;
        this.IsDown = true;
      });
    } else {
