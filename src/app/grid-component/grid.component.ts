...
export class GridComponent implements OnInit {
  ...
  ngOnInit() {
    if (this.getChromeVersion() >= 85) {
      const sheet = window.document.styleSheets[0] as CSSStyleSheet;
      sheet.insertRule(
        '.ag-body-viewport { direction: ltr; }',
        sheet.cssRules.length
      );
      sheet.insertRule(
        '.ag-cell-wrapper { direction: rtl; }',
        sheet.cssRules.length
      );
      sheet.insertRule(
        '.ag-body-horizontal-scroll-viewport { direction: ltr; }',
        sheet.cssRules.length
      );
      sheet.insertRule(
        '.ag-center-cols-container { display: inline-grid; }',
        sheet.cssRules.length
      );
    }
  ...
  onRowDataChanged() {
    ...
    if (this.gridApi) {
      ...
      if (this.getChromeVersion() >= 85) {
        for (let index = 0; index < $('.ag-body-horizontal-scroll-viewport').length; index++) {
          $('.ag-body-horizontal-scroll-viewport')[index].scrollLeft = 60000;
        }
      }
    }
  }
